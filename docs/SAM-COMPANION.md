...
## Perspective depth refinement

Sam remains the original SVG character. This is 2.5D CSS perspective, not a replacement 3D mesh or a new renderer. `pet-depth.js` decorates existing XY/time samples with a smooth Z excursion. Flights alternate nearer/farther, with depth proportional to travel distance; settled speech perches remain exactly at Z=0. Desktop bounds are +80/-200px at a 640px perspective distance; phone depth is milder at +50/-160px. No flips, face changes, transparency fading or speech-box scaling.

The flight wrapper uses local perspective after screen-space translation, so depth does not pull the destination toward the viewport center. Existing parabolic geometry and duration pass through unchanged. Final animation effects are cancelled before measuring speech placement, since timer and compositor clocks can otherwise differ by a frame. Pause/hide still reparent Sam to his normal dock.
