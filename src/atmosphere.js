...
export async function startAtmosphere(host,control){
 ...
 const reduce=matchMedia('(prefers-reduced-motion: reduce)');let paused=reduce.matches||document.documentElement.dataset.motionPaused==='true';
 const fail=(e)=>{if(failed)return;failed=true;cancelAnimationFrame(raf);host.dataset.renderer='fallback';host.dataset.error=String(e?.message||'GPU unavailable');gpu?.dispose();};
 try{
  ...
  const draw=()=>{shader.set({params:{resolution:output.size,time,activity}});const age=(performance.now()-burstAt)/1000;host.dataset.burstAge=age.toFixed(3);frame(gpu,f=>f.pass(output,shader));host.dataset.frames=String(Number(host.dataset.frames||0)+1);};
  const tick=(now)=>{raf=0;if(failed||paused||document.hidden)return;const responding=host.dataset.state==='researching';const typing=now<typingUntil;host.dataset.thinking=responding?'responding':typing?'typing':'idle';const interval=responding||typing?50:83.333;if(!previous||now-previous>=interval){const dt=previous?Math.min((now-previous)/1000,.2):1/12;previous=now;const goal=responding?1:typing?.35:0;activity+=(goal-activity)*(1-Math.exp(-dt*3));time+=dt*(1+3*activity);host.dataset.activity=activity.toFixed(4);draw();}raf=requestAnimationFrame(tick);};
  const sync=()=>{cancelAnimationFrame(raf);previous=0;if(failed||document.hidden)return;if(paused){activity=0;host.dataset.activity='0';draw();}else raf=requestAnimationFrame(tick);};
  ...
  window.addEventListener('sam-motion',e=>{paused=e.detail.paused;sync();});document.addEventListener('visibilitychange',sync);addEventListener('resize',()=>{resize();if(paused)draw();},{passive:true});addEventListener('pagehide',()=>cancelAnimationFrame(raf));addEventListener('pageshow',sync);new MutationObserver(()=>{if(host.dataset.state==='researching')burstAt=performance.now();if(host.dataset.state!=='idle')typingUntil=0;if(paused&&!failed)sync();}).observe(host,{attributes:true,attributeFilter:['data-state']});sync();
