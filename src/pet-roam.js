...
import { flightTransform, planPetFlyby } from './pet-depth.js';
...
export function roamPet(stage, pet) {
  ...
  const speechTail=document.createElementNS('http://www.w3.org/2000/svg','svg');speechTail.classList.add('pet-speech-tail');speechTail.setAttribute('aria-hidden','true');speechTail.style.display='none';
  flight.append(pet); document.body.append(flight, speechTail, bubble);
  ...
  let lastFlyby=performance.now()-8000,flybyTrip=0;
  let speechPointer={x:-Infinity,y:-Infinity},speechHoldUntil=0;
  ...
  const moveTo = (target, reason = 'focus', immediate = false) => {
    ...
    hideBubble();target=contentTarget(target);const p = edgePerch(target); destination = p; setDocked(false); clearTimeout(moveTimer);
    ...
    const distance = Math.hypot(p.x - from.x, p.y - from.y);if(!immediate&&distance<18){p.x=from.x;p.y=from.y;}const journey=planPetFlight(from,p,{width:innerWidth,height:innerHeight,trip:flightTrip++,quiet:editing()});const duration=immediate||distance<18?0:journey.duration;if(!immediate){p.x=journey.points.at(-1).x;p.y=journey.points.at(-1).y;}flight.dataset.path=immediate?'perch':journey.kind;flight.dataset.depth=duration?journey.depthKind:'rest';
    ...
    if (duration) flightAnimation = flight.animate(journey.points.map(point=>({transform:flightTransform(point),offset:point.offset})), { duration, easing: 'linear' });
    flight.style.transform = flightTransform(p); lastMove = performance.now();
    moveTimer = setTimeout(() => { flightAnimation?.cancel();flightAnimation=undefined;const r=target?.getBoundingClientRect?.(); if(r){pet.style.setProperty('--look-x',`${clamp((r.left+r.width/2-p.x-35)/80,-3,3)}px`);pet.style.setProperty('--look-y',`${clamp((r.top+r.height/2-p.y-40)/100,-2,2)}px`);} flight.dataset.flying = 'false';flight.dataset.depth='rest'; emitter.active = false; if(pendingMove)return; if (reason === 'idle') showInvite(); else if(reason==='chat')sayAside('chat',true);else if (reason === 'focus' && target?.closest?.('.breakout,.showcase-card,#services')) showArrival(target); }, duration);
  };
  const readingSpeech=()=>{if(bubble.hidden)return false;const r=bubble.getBoundingClientRect();return speechPointer.x>=r.left-12&&speechPointer.x<=r.right+12&&speechPointer.y>=r.top-12&&speechPointer.y<=r.bottom+12;};
  const protectedSpeech=()=>!bubble.hidden&&(readingSpeech()||performance.now()<speechHoldUntil||performance.now()-lastBubble<12000);
  const queueMove=(target,reason)=>{
    pendingMove={target,reason};clearTimeout(retargetTimer);
    const settle=()=>{if(!usable()){pendingMove=null;return;}if(flight.dataset.flying==='true'||protectedSpeech()){retargetTimer=setTimeout(settle,250);return;}const next=pendingMove;pendingMove=null;if(next)moveTo(next.target,next.reason);};
    retargetTimer=setTimeout(settle,reason==='scroll'?850:650);
  };

  const hideBubble = () => { bubble.hidden = true;speechHoldUntil=0; speechTail.style.display='none'; flight.dataset.point = 'false'; input?.classList.remove('dot-invite'); };
  const placeSpeech=()=>{
    ...
    const candidates=[];for(const x of [r.right+12,r.left-w-12,r.left+r.width/2-w/2])for(const y of [r.top,r.top-h-12,r.bottom+12,...[24,36,48,60,84].map(g=>r.top-h-g),r.bottom+36,r.bottom+60])candidates.push({x:clamp(x,8,innerWidth-w-8),y:clamp(y,8,innerHeight-h-8)});
    ...
    const cx=r.left+r.width/2,cy=r.top+r.height*.42;let sx=clamp(cx,place.x+12,place.x+w-12),sy=clamp(cy,place.y+12,place.y+h-12),vertical=false;
    if(cx<place.x){sx=place.x;vertical=true;}else if(cx>place.x+w){sx=place.x+w;vertical=true;}else sy=cy<place.y?place.y:place.y+h;
    const distance=Math.hypot(sx-cx,sy-cy)||1,ex=cx+(sx-cx)/distance*r.width*.43,ey=cy+(sy-cy)/distance*r.width*.43;
    speechTail.setAttribute('viewBox',`0 0 ${innerWidth} ${innerHeight}`);speechTail.innerHTML=`<path d="M ${sx+(vertical?0:-4)} ${sy+(vertical?-4:0)} Q ${(sx+ex)/2} ${(sy+ey)/2-3} ${ex} ${ey} Q ${(sx+ex)/2} ${(sy+ey)/2+3} ${sx+(vertical?0:4)} ${sy+(vertical?4:0)} Z"/>`;speechTail.style.display='';
  };
  ...
  const focusOrHover = e => { if(e.type==='pointerover'&&performance.now()-lastDodge<1400)return;const target = e.target.closest?.(INTERACTIVE); if (!target || target.closest('.dot-flight,.dot-bubble,header,.handoff')||!target.closest('main') || (e.type==='pointerover' && target === activeTarget && performance.now() - lastMove < 1200)) return; if(e.type==='focusin'&&target===input)lastChatFocus=performance.now();activeTarget = target; clearTimeout(scrollTimer); if(e.type==='pointerover')lastActivity=performance.now();else noteActivity(); if(e.type==='focusin'&&target===input&&flight.dataset.flying==='true'){clearTimeout(retargetTimer);pendingMove=null;moveTo(input,'chat',true);return;}queueMove(target, target.closest('#composer') ? 'chat' : 'focus'); };
  ...
  const speechIntervention=event=>{if(event.pointerType!=='mouse')return;const wasReading=readingSpeech();speechPointer={x:event.clientX,y:event.clientY};if(wasReading||readingSpeech())speechHoldUntil=performance.now()+4000;};
  const leaveSpeech=event=>{if(!event.relatedTarget){if(readingSpeech())speechHoldUntil=performance.now()+4000;speechPointer={x:-Infinity,y:-Infinity};}};
  const tickle = event => {
    if(event.pointerType!=='mouse'||!usable()||protectedSpeech()||editing()||document.querySelector('#intro')||performance.now()-lastDodge<4500||flight.dataset.flying==='true')return;
    ...
    const start=flight.getBoundingClientRect(),journey=planPetFlight({x:start.left,y:start.top},end,{width:innerWidth,height:innerHeight,trip:1,quiet:true});const landing=journey.points.at(-1);destination={...destination,x:landing.x,y:landing.y};
    flight.dataset.flying='true';flight.dataset.path='tickle';flight.dataset.depth=journey.depthKind;flight.dataset.dodges=String(Number(flight.dataset.dodges||0)+1);flight.style.setProperty('--lean',end.x>start.left?'12deg':'-12deg');
    flightAnimation=flight.animate(journey.points.map(p=>({transform:flightTransform(p),offset:p.offset})),{duration:1500,easing:'linear'});flight.style.transform=flightTransform(landing);moveTimer=setTimeout(()=>{flightAnimation?.cancel();flightAnimation=undefined;flight.dataset.flying='false';flight.dataset.depth='rest';sayAside('tickle');},1500);
  };

  const playfulFlyby=()=>{
    const now=performance.now();if(!usable()||editing()||pendingMove||protectedSpeech()||flight.dataset.flying==='true'||document.querySelector('#intro')||now-lastActivity<2500||now-lastMove<6000||now-lastBubble<18000||now-lastFlyby<24000)return false;
    lastFlyby=now;hideBubble();clearTimeout(moveTimer);const rect=flight.getBoundingClientRect(),from={x:rect.left,y:rect.top};
    const journey=planPetFlyby(from,{width:innerWidth,height:innerHeight,trip:flybyTrip++});const priorIntent=flight.dataset.intent;
    flight.dataset.path='flyby';flight.dataset.depth='near';flight.dataset.flying='true';flight.dataset.intent='showing-off';flight.dataset.flybys=String(flybyTrip);flight.style.setProperty('--lean',flybyTrip%2?'12deg':'-12deg');
    flightAnimation?.cancel();flightAnimation=flight.animate(journey.points.map(point=>({transform:flightTransform(point),offset:point.offset})),{duration:journey.duration,easing:'linear'});flight.style.transform=flightTransform(from);lastMove=now;
    moveTimer=setTimeout(()=>{flightAnimation?.cancel();flightAnimation=undefined;flight.dataset.flying='false';flight.dataset.depth='rest';flight.dataset.intent=priorIntent||'listening';if(!pendingMove)sayAside('general');},journey.duration);return true;
  };
  ...
  let emitterPrevious;const bubbleStop = createBubbleField(document.body, () => {const now=performance.now(),r=pet.getBoundingClientRect();const cx=r.left+r.width/2,cy=r.top+r.height*.78;if(emitterPrevious&&now>emitterPrevious.t){const dt=Math.max(.008,(now-emitterPrevious.t)/1000);emitter.vx=clamp((cx-emitterPrevious.x)/dt,-500,500);emitter.vy=clamp((cy-emitterPrevious.y)/dt,-500,500);}emitterPrevious={x:cx,y:cy,t:now};emitter.x=cx;emitter.y=cy;emitter.podOffset=r.width*.33;emitter.scale=clamp(flight.getBoundingClientRect().width/(flight.offsetWidth||82),.5,2.8);emitter.suspended=!usable()||!!document.querySelector('#intro');emitter.moving=flight.dataset.flying==='true';emitter.active=usable();return emitter;});
  document.addEventListener('focusin', focusOrHover); document.addEventListener('pointerover', focusOrHover, { passive: true });document.addEventListener('pointermove',speechIntervention,{passive:true});document.addEventListener('pointermove',tickle,{passive:true});document.addEventListener('pointerout',leaveSpeech,{passive:true}); document.addEventListener('pointerdown', noteActivity, { passive: true }); document.addEventListener('keydown', noteActivity);document.addEventListener('input',onInput); document.addEventListener('scroll', onScroll, { passive: true }); addEventListener('resize', onScroll, { passive: true }); reduced.addEventListener('change', sync); document.addEventListener('visibilitychange', sync); document.addEventListener('toggle', sync, true); addEventListener('pagehide', () => { emitter.active = false; setDocked(true); }); addEventListener('pageshow', sync);
  sync();
  ...
  const chatter=setInterval(()=>{if(performance.now()-lastActivity<1800)return;if(playfulFlyby())return;const mood=stage.dataset.mood;const kind=flight.dataset.targetKind;const context=mood==='thinking'?'thinking':mood==='reply'?'reply':kind==='services'?'services':kind==='breakout'?'game':kind==='showcase'?'design':kind==='chat'?'chat':document.querySelector('.turn.assistant')?'reply':'general';sayAside(context);},1000);
  const idle = setInterval(() => { if (usable() && document.activeElement !== input && rectVisible(input?.getBoundingClientRect()) && stage.dataset.mood === 'idle' && !idleHinted && !protectedSpeech() && performance.now()-lastBubble>=12000 && performance.now() - lastActivity > 15000) { idleHinted = true; queueMove(input || stage, 'idle'); } }, 2000);
  return () => { disposed = true; clearTimeout(helloTimer);clearTimeout(retargetTimer);pendingMove=null; clearInterval(chatter); clearInterval(idle); clearTimeout(scrollTimer); clearTimeout(moveTimer); hideBubble(); flightAnimation?.cancel(); moodObserver.disconnect(); hiddenObserver.disconnect(); bubbleStop(); stage.prepend(pet); flight.remove(); bubble.remove();speechTail.remove(); document.removeEventListener('focusin', focusOrHover); document.removeEventListener('pointerover', focusOrHover);document.removeEventListener('pointermove',tickle);document.removeEventListener('pointermove',speechIntervention);document.removeEventListener('pointerout',leaveSpeech); document.removeEventListener('pointerdown', noteActivity); document.removeEventListener('keydown', noteActivity);document.removeEventListener('input',onInput); document.removeEventListener('scroll', onScroll); removeEventListener('resize', onScroll); reduced.removeEventListener('change', sync); document.removeEventListener('visibilitychange', sync); document.removeEventListener('toggle', sync, true); removeEventListener('pageshow', sync); };
}
