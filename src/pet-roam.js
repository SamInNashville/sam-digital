...
export function roamPet(stage, pet) {
  ...
  const focusOrHover = e => { const target = e.target.closest?.(INTERACTIVE); if (!target || target.closest('.dot-flight,.dot-bubble,header,.handoff')||!target.closest('main') || (target === activeTarget && performance.now() - lastMove < 1200)) return; activeTarget = target; clearTimeout(scrollTimer); noteActivity(); moveTo(target, target.closest('#composer') ? 'chat' : 'focus'); };
  ...
  sync();
  const helloTimer=setTimeout(()=>{if(!usable()||document.activeElement===input||!rectVisible(stage.getBoundingClientRect()))return;moveTo(stage,'welcome',true);bubble.textContent="I'm Sam! I'll guide you.";placeSpeech();bubble.hidden=false;bubbleTimer=setTimeout(hideBubble,6500);},2300);
