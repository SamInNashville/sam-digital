...
import { flightTransform } from './pet-depth.js';
...
export function roamPet(stage, pet) {
  ...
  let emitterPrevious;const bubbleStop = createBubbleField(document.body, () => {const now=performance.now(),r=pet.getBoundingClientRect();const cx=r.left+r.width/2,cy=r.top+r.height*.78;if(emitterPrevious&&now>emitterPrevious.t){const dt=Math.max(.008,(now-emitterPrevious.t)/1000);emitter.vx=clamp((cx-emitterPrevious.x)/dt,-500,500);emitter.vy=clamp((cy-emitterPrevious.y)/dt,-500,500);}emitterPrevious={x:cx,y:cy,t:now};emitter.x=cx;emitter.y=cy;emitter.podOffset=r.width*.33;emitter.scale=clamp(flight.getBoundingClientRect().width/(flight.offsetWidth||82),.65,1.25);emitter.suspended=!usable()||!!document.querySelector('#intro');emitter.moving=flight.dataset.flying==='true';emitter.active=usable();return emitter;});
