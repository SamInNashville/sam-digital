...
export function startCompanion(host){
 ...
 const setState=value=>{stage.dataset.mood=value;if(label)label.textContent=value==='thinking'?'Sam is thinking locally':value==='listening'?'Sam is listening':value==='reply'?'Sam has a reply':'Sam · your browser AI';};
 ...
 toggle.addEventListener('click',()=>{hidden=!hidden;pet.hidden=hidden;toggle.textContent=hidden?'Show Sam':'Hide Sam';toggle.setAttribute('aria-expanded',String(!hidden));sync();});
