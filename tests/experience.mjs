...
try{
 await page.goto(base);await expect(page.locator('#intro')).toHaveCount(0);await expect(page.locator('#showcase canvas')).toBeVisible();await expect(page.getByText('Meet Dot. AI, not a human.',{exact:true})).toBeVisible();await expect(page.locator('#companion')).toBeVisible();
 ...
 await page.locator('#motion').click();await page.evaluate(()=>window.finishReply());await expect(page.locator('.speaker')).toContainText('DOT · BROWSER AI');await expect(page.locator('#companion-stage')).toHaveAttribute('data-mood','reply');await page.waitForTimeout(5000);
 await page.locator('#companion-toggle').click();await expect(page.locator('#companion')).toBeHidden();await page.locator('#companion-toggle').click();await expect(page.locator('#companion')).toBeVisible();
