...
async function sampleFlight(label,interrupt=false){
 ...
 await p.waitForTimeout(1250);await p.screenshot({path:`proof/sam-depth/${label}-${mode}.png`});if(interrupt){await p.locator('#prompt').focus();await p.keyboard.type('I am writing now.');}const rows=await pending;
...
try{
 ...
 await p.locator('#showcase').scrollIntoViewIfNeeded();await p.locator('[data-demo=design]').focus();await expect(p.locator('.dot-flight')).toHaveAttribute('data-target-kind','showcase',{timeout:8000});await expect(p.locator('.dot-flight')).toHaveAttribute('data-flying','false',{timeout:8000});
 for(const [label,target] of [['first','[data-demo=arcade]'],['second','[data-demo=design]']]){await p.locator(target).focus();await sampleFlight(label);}
 const far=results.find(r=>r.mode==='far'),near=results.find(r=>r.mode==='near');expect(far).toBeTruthy();expect(near).toBeTruthy();expect(far.minZ).toBeLessThan(-400);expect(far.minScale).toBeLessThan(.7);expect(far.minEmitter).toBeLessThan(.7);expect(near.maxZ).toBeGreaterThan(300);expect(near.maxScale).toBeGreaterThan(2);expect(near.maxEmitter).toBeGreaterThan(2);
 await expect(p.locator('.dot-flight')).toHaveAttribute('data-path','flyby',{timeout:45000});const play=await sampleFlight('flyby',true);expect(play.maxScale).toBeGreaterThan(2);await expect(p.locator('#prompt')).toBeFocused();await expect(p.locator('#prompt')).toHaveValue('I am writing now.');await expect(p.locator('.dot-bubble')).toBeHidden();
 await p.locator('[data-demo=arcade]').focus();await expect(p.locator('.dot-flight')).toHaveAttribute('data-target-kind','breakout',{timeout:8000});await expect(p.locator('.dot-flight')).toHaveAttribute('data-flying','false',{timeout:8000});
 ...
 await writeFile('proof/sam-depth/metrics.json',JSON.stringify(results,null,2));console.log('PASS bold close-up/retreat, automatic flyby with typing interruption, depth-matched emitter, resting speech scale, mobile bounds and pause');console.log(JSON.stringify(results));
