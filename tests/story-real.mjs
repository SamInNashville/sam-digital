...
try{
 ...
 for(const question of questions){await page.evaluate(()=>window.rawModel=[]);await page.locator('#prompt').fill(question);await page.locator('#composer').evaluate(f=>f.requestSubmit());await expect(page.locator('#atmosphere')).toHaveAttribute('data-state','engaged',{timeout:180000});const answer=await page.locator('.answer-text').last().innerText();const raw=await page.evaluate(()=>window.rawModel.join(''));const record=await page.evaluate(()=>JSON.parse(sessionStorage.getItem('sam-digital-story-v1')));results.push({question,answer,raw,assessment:record.assessment});console.log(JSON.stringify(results.at(-1)));}
 ...
 if(!process.env.DIAGNOSTIC){expect(results[0].answer).toContain('Nashville');expect(results[0].assessment.reason).not.toBe('ready');expect(results[1].assessment.reason).not.toBe('ready');expect(results[2].assessment.reason).toBe('ready');expect(results[3].answer).not.toContain('not sure I can');expect(results[4].assessment.evidence.people?.quote||'').not.toContain('parents');expect(results[5].assessment.reason).toBe('uncertain');}
