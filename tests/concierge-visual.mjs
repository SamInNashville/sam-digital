...
try{
 ...
 await page.getByRole('button',{name:'What do you do?',exact:true}).click();await page.waitForTimeout(900);await expect(page.locator('#atmosphere')).toHaveAttribute('data-state','researching');const network=await pixels();expect(network).not.toBe(a);await page.screenshot({path:'proof/concierge/synapses.png'});await page.waitForTimeout(700);expect(await pixels()).not.toBe(network);
 ...
 await writeFile('proof/concierge/visual-results.json',JSON.stringify({renderer:'real vgpu',model:'held test fixture',idlePixelsChange:true,synapsePixelsChange:true,pauseStable:true,mobileNoOverflow:true,reducedMotion:true},null,2));
 console.log('PASS real vgpu smoke, travelling synapses, pause, reduced motion and mobile capture');
